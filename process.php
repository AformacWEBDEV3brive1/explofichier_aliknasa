...
function envoyer() {
    ...
    for ($i = 0; $i < count($dossier); $i++) {
        ...
        if (is_dir($pathDossier)) {
            ...
            echo "<div id='" . $dossier[$i] . "' class='folder ligne col-md-3'onclick='clickDossier(this.id)'data-toggle='tooltip()' title='Ce fichier a été mofifié le : " . date('F d Y H:i:s', filemtime($pathDossier)) . "'><i class='fa fa-2x fa-file-o'></i><p>${dossier[$i]}</p></div>";
        }
    }
}
...
function creation() {
    ...
    $command = 'touch /home/boul/test/' . $element . '.txt';
    $command = shell_exec('touch /tmp/toghfgfhhgfhhto 2>&1');
    print_r($command);
}
