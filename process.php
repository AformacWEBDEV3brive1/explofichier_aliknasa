...
function clickRetour() {
    ...
    for ($i = 0; $i < count($tabl_dossier); $i++) {
        ...
        if (is_dir($pathDossier)) {
            ...
            echo "<div id='" . $tabl_dossier[$i] . "' data-toggle='tooltip()' title='Ce fichier a été mofifié le : " . date('F d Y H:i:s', filemtime($pathDossier))."' class='folder ligne col-md-3'><i class='fa fa-2x fa-file-o'></i><p>${tabl_dossier[$i]}</p></div>";
        }
    }
}
?>
